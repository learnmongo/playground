...
const getIngredient = () => {
  ...
  const origin = ['france', 'japan', 'mexico'][falso.randNumber({ min: 0, max: 2 })]
...
const getDirections = (steps = 5) => falso.randTextRange({ min: 10, max: 50, length: steps });

const getRatings = () => falso.randNumber({ min: 1, max: 5, length: falso.randNumber({ min: 0, max: 5 }) });
